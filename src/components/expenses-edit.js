...
export class EditExpense {
    ...
    async editCategory() {
        if (this.title) {
            try {
                this.result = await CustomHttp.request(config.host + '/categories/expense/' + this.params.id, 'PUT', {
                    title: this.title,
                });
                if (this.result) {
    ...
    async getCategory() {
        try {
            this.result = await CustomHttp.request(config.host + '/categories/expense/' + this.params.id);
            if (this.result) {
                if (this.result.error) {
                    throw new Error(result.error);
                }
