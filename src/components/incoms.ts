...
export class Incoms {
    ...
    private showIncoms(): void {
        (this.result as CategoriesIncomeType[]).forEach(element => {
            ...
            buttonEdit.setAttribute('href', '#/incoms-edit?id=' + element.id);
    ...
    private async deleteCategory(): Promise<void> {
        try {
            const res: DefaultResponseType = await CustomHttp.request(config.host + '/categories/income/' + this.cardId, "DELETE",
            );
            if (res) {
                if (res.error) {
