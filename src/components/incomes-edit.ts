...
export class EditIncome {
    ...
    private async editCategory(): Promise<void> {
        if (this.title) {
            try {
                let result = await CustomHttp.request(config.host + '/categories/income/' + this.params.id, 'PUT', {
                    title: this.title,
                });
                if (result) {
                    if (result.error) {
    ...
    private async getCategory(): Promise<void> {
        try {
            let result = await CustomHttp.request(config.host + '/categories/income/' + this.params.id);
            if (result) {
                if (result.error) {
                    throw new Error(result.error);
                }
